from src.parser.map_parser import parse_metadata
from rich import print
from src.model.graph import build_graph, shortest_path
from src.simulation.scheduler import build_timeline, format_output


if __name__ == "__main__":

    aaa = parse_metadata('''# Hard Level 2:\
Extreme capacity constraints with timing challenges
nb_drones: 12

start_hub: start 0 -1 [color=green]
end_hub: goal 9 0 [color=green]
...
hub: priority_bypass1 3 2 [zone=priority color=cyan max_drones=3]
hub: priority_bypass2 4 2 [zone=priority color=cyan max_drones=3]
...
connection: waiting_area2-priority_bypass2
connection: priority_bypass1-priority_bypass2
connection: priority_bypass2-convergence [max_link_capacity=2]

# Final challenge
connection: convergence-final_bottleneck [max_link_capacity=3]
...
''')
    ...
    path = shortest_path(test.start, test.end, test)

    # print(build_graph(aaa).zones, build_graph(aaa).connections)
    for i in format_output({
        "1": build_timeline(path),
        "2": build_timeline(path)
            }):
        print(i)
